Page({
  data: {
    ...
    showTotalTime: false,
    showGo: false,
    buttonIcon: 'amrap-start.png', // 初始按钮图标
    soundEnabled: true, // 声音开关状态
    intervalId: null, // 用来存储倒计时的interval
    dingAudio: null,
    dengAudio: null,
    incrementCount: 0
  },
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      currentTimeLeft: this.formatTime(targetTime),
    })
    this.updateProgress();
  },
  ...
  toggleTimer() {
    if (this.data.totalTime == 0) {
      this.setData({
        ...
        workstate: true,
        incrementCount: 0,
        totalTime: this.data.targetTime
      });
      this.startCountdown();
    } else {
  ...
  pauseTimer() {
    ...
    this.setData({
      ...
      buttonIcon: 'amrap-start.png' // 恢复按钮图标
    });
  },

  updateProgress() {
    if (this.data.showTotalTime) {
      const left = this.data.totalTime;
      ...
      this.setData({
        currentTimeLeft: this.formatTime(1000 + left),
        value: ((this.data.targetTime - left) / this.data.targetTime) * 100
      });
    }
  },
